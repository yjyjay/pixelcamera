    ...
    let colorMode = '64'; // Default changed to 64 (removed 16)
    let ditherType = 'bayer'; // 'bayer' (Ordered) or 'error' (Floyd-Steinberg)
    ...
    // --- Helper Functions (Defined before usage to prevent hoisting issues) ---
    ...
    // Updated getQuantizedColor with Input Clamping
    function getQuantizedColor(r, g, b) {
        ...
        if (colorMode === 'bw') {
            ...
            // 64 Colors (2-bit per channel: 4x4x4)
            r = Math.round(r / 85) * 85;
            g = Math.round(g / 85) * 85;
            b = Math.round(b / 85) * 85;
            return [r, g, b];
        } else if (colorMode === '128') {
            // 128 Colors (R:4 x G:8 x B:4)
            // Green gets more levels because human eye is more sensitive to it
            r = Math.round(r / 85) * 85;        // 4 levels
            g = Math.round(g / 36.43) * 36.43;  // 8 levels (255/7 ≈ 36.43)
    ...
    function renderPreview() {
        if (video.readyState === video.HAVE_ENOUGH_DATA) {
            ...
            if (ditherType === 'bayer') {
                // === Standard Bayer 4x4 Ordered Dithering ===
                ...
                // === Error Diffusion (Floyd-Steinberg) OR Special 2x2 Bayer for Blue ===
                
                if (colorMode === '4') {
                    // SPECIAL OVERRIDE: For Blue (4-color) mode in Diffusion setting, use Bayer 2x2
                    // This provides clearer results than standard diffusion for low color counts.
                    for (let y = 0; y < currentResolution; y++) {
                        for (let x = 0; x < currentResolution; x++) {
                            const index = (y * currentResolution + x) * 4;
                            // 2x2 Matrix: 0..3 values. 
                            // (Value - 1.5) * 25 creates a range approx -37.5 to +37.5
                    ...
                    // Standard Floyd-Steinberg for other modes
